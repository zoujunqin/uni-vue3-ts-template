<template>
  ...
  >
    ...
    >
      ...
      <slot name="desc">
        <ProText :text="desc" type="tips" size="12" />
      </slot>
    </view>
  </view>
</template>
...
export default {
  ...
  computed: {
    lineStyle() {
      ...
      if (this.parent.children?.[this.index + 1]?.error) {
        style.backgroundColor = '#f56c6c';
      } else {
        if (this.index === Number(this.parentData.current)) {
          style.background = `linear-gradient(90deg, ${this.parentData.activeColor} 50%, #E0EDFF 50%)`;
        } else if (this.index <= this.parentData.current) {
          style.background = `linear-gradient(90deg, ${this.parentData.activeColor} 50%, ${this.parentData.activeColor} 100%)`;
        } else {
          style.background = `linear-gradient(90deg, #E0EDFF 50%, #E0EDFF 100%)`;
        }
      }
    ...
    contentStyle() {
      ...
      if (this.parentData.direction === 'column') {
        ...
        style.marginTop = this.parentData.dot ? '0px' : '4px';
      } else {
        style.marginTop = this.parentData.dot ? '2px' : '4px';
        style.marginLeft = this.parentData.dot ? '2px' : '6px';
      }
      return style;
    }
...
@import '@climblee/uv-ui/libs/css/color.scss';
