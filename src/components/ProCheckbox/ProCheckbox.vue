<template>
  <view
    ...
    :class="[
      `uv-checkbox-label--${parentData.iconPlacement}`,
  ...
  >
    ...
    <view class="uv-checkbox__label-wrap" @tap.stop="labelClickHandler">
      <slot>
        <text
          :style="{
  ...
  computed: {
    ...
    checkboxStyle() {
      ...
      if (this.parentData.borderBottom && this.parentData.placement === 'row') {
        this.$uv.error(
          '检测到您将borderBottom设置为true，需要同时将uv-checkbox-group的placement设置为column才有效'
        );
      }
